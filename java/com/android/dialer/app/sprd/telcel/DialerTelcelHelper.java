...
public class DialerTelcelHelper {
    ...
    public interface OnFdnCacheRefreshListener {
         void OnFdnCacheRefresh();
    }

    private DialerTelcelHelper() {
    }

    private static DialerTelcelHelper mInstance;

    public static DialerTelcelHelper getInstance() {
        if (mInstance != null) {
            return mInstance;
        }
    ...
    public String queryFdnCache(Context context, CharSequence number, int subId, CharSequence name) {
        ...
        /**UNISOC:modify the bug for 1153184 & 1202781@{*/
        if (fdnName == null && cache != null && cache.size() > 0) {
            String numberWithoutCountryCode = InCallUiUtils.getPhoneNumberWithoutCountryCode(number.toString(), context);
            fdnName = cache.get(numberWithoutCountryCode);
        }
        /* @}*/
            ...
            for (int i = 0; i < phoneCount; i++) {
                ...
                if (subscriptionInfo != null) {
                    ...
                    if (TelephonyManagerEx.from(context).getIccFdnEnabled(subId) && cache != null
                        ...
                        /**UNISOC:modify the bug for 1153184 @{*/
                        if (fdnName == null) {
                            String numberWithoutCountryCode = InCallUiUtils.getPhoneNumberWithoutCountryCode(number, context);
                            fdnName = cache.get(numberWithoutCountryCode);
                        }
                        /* @}*/
                        if (fdnName != null) {
