...
public class InCallFragment extends Fragment
  ...
  private final Runnable pagerRunnable =
      new Runnable() {
        @Override
        public void run() {
          if (adapter != null) {//UNISOC:add for bug1570243
  ...
  public void onPageChanged(int buttonsToPlaceSize) {
    ...
    if (this.buttonsToPlaceSize != buttonsToPlaceSize && adapter != null
      ...
      try {
        ...
        //UNISOC:add for bug1398180
        try {
          adapter.notifyDataSetChanged();
        }  catch (IllegalStateException newE) {
          LogUtil.e("InCallFragment.onPageChanged", "new IllegalStateException : " + newE);
        }
        return;
        /* @} */
      }
