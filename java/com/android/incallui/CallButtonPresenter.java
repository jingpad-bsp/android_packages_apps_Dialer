...
public class CallButtonPresenter
  ...
  public static final Uri PAUSE_IMAGE_URI = Uri.parse("file:///system/etc/vilte/vertical_480.raw");
  ...
  public void addCallClicked() {
    ...
    //UNISOC: add for bug1268262
    dismissKeyguardIfNeeded(0);
  }

  /*UNISOC: add for bug1268262  1298932 */
  private void dismissKeyguardIfNeeded(final int type) {
    KeyguardManager keyguardManager = (KeyguardManager) context.getSystemService(KeyguardManager.class);
    InCallActivity inCallActivity = getActivity();
    if (keyguardManager != null && keyguardManager.isKeyguardLocked() && inCallActivity != null) {
      LogUtil.i("CallButtonPresenter", "  dismissKeyguardIfNeeded");
      keyguardManager.requestDismissKeyguard(inCallActivity, new KeyguardManager.KeyguardDismissCallback() {
        @Override
        ...
        public void onDismissSucceeded() {
          ...
          CallButtonPresenter.this.onDismissSucceeded(type);
        }
      ...
      onDismissSucceeded(type);
    }
  }
  /*UNISOC: add for bug1298932 */
  private void onDismissSucceeded(int type) {
    if (type == 0) {
      addCallFunction();
    } else {
      invite();
  ...
  public void changeToVideoClicked() {
    ...
    Logger.get(context)
        .logCallImpression(
            ...
            call.getTimeAddedMs());
    //UNISOC: add for bug1510544 video customer service
    if (isSupportTxRxVideo(call) && !InCallUiUtils.isSupportVideoCustomerService(context, call)) {
  ...
  public void pauseVideoClicked(boolean pause) {
    ...
    if (pause) {
      ...
      //UNISOC: add for bug1510544 video customer service
      inCallButtonUi.showButton(InCallButtonIds.BUTTON_SWITCH_CAMERA, false);
      if(InCallUiUtils.isSupportVideoCustomerServiceAndNumbers(context, call)){
        call.getVideoTech().setPauseImage(PAUSE_IMAGE_URI);
      }/*else {
      ...
      updateCamera(
          InCallPresenter.getInstance().getInCallCameraManager().isUsingFrontFacingCamera());
      //UNISOC: add for bug1510544 video customer service
      inCallButtonUi.showButton(InCallButtonIds.BUTTON_SWITCH_CAMERA, true);
      if(InCallUiUtils.isSupportVideoCustomerServiceAndNumbers(context, call)){
        call.getVideoTech().setPauseImage(null);
      }/*else {
  ...
  private void updateButtonsState(DialerCall call) {
    ...
    //UNISOC:add for bug958670 Control show change Video type list
    final boolean isSupportTxRxVideo = isSupportTxRxVideo(call)
            && !InCallUiUtils.isSupportVideoCustomerService(context, call);//UNISOC: add for bug1510544 video customer service
   ...
   /* UNISOC: Add for change video type feature @{ */
    ...
    final boolean showPauseVideo =
          !isSupportTxRxVideo// Add for change video type feature
            ...
            && !(VideoUtils.isRxOnlyVideoCall(call) //UNISOC: add for bug1510544 video customer service
                  && InCallUiUtils.isSupportVideoCustomerServiceAndNumbers(context, call));
    ...
    final boolean canInvite = isConferenceCall
            && UserManagerCompat.isUserUnlocked(context) // UNISOC: modify for bug1298932
  ...
  public void inviteClicked() {
    ...
    // UNISOC: modify for bug1298932
    dismissKeyguardIfNeeded(-1);
  }
  /*UNISOC: modify for bug1298932 */
  private void invite() {
    ...
    public void onReceive(Context context, Intent intent) {
      if (Intent.ACTION_USER_PRESENT.equals(intent.getAction())) {
        ...
        final boolean canInvite = call.isConferenceCall()
                // UNISOC: add for bug 895929
                && call.getChildCallIds().size() >= 1
                && (call.getChildCallIds().size() < 5)
                && mStayVolte && (call.getState() == DialerCallState.ACTIVE)
                // UNISOC: add for bug920540
                && !call.isVideoCall() && ImsManagerEx.isImsRegisteredForPhone(InCallUiUtils.getPhoneIdByAccountHandle(context, call.getAccountHandle()))
                // add for bug966900
                && !call.hasProperty(android.telecom.Call.Details.PROPERTY_WIFI)
                // modify for bug 1138221
                && InCallUiUtils.ShowInviteButton(context,call);
