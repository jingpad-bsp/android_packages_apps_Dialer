    ...
    if (call.getState() == DialerCallState.DISCONNECTED) {
      ...
      if (callById.containsKey(call.getId())) {
        ...
        try {  //UNISOC: modify for bug1231482
          ...
          LogUtil.d("DialerCallListenerImpl.updateCallInMap", "call.getDisconnectCause().getCode():"
            + call.getDisconnectCause().getCode() + ", delayTimes :" + delayTimes + ",isDelayFinish:"
            + isDelayFinish + ",notFinishUi:" + notFinishUi);
          if ((delayTimes > 1 && !isDelayFinish && (call.getDisconnectCause().getCode()
              == DisconnectCause.LOCAL)) || !notFinishUi) {
