...
public class InCallUiUtils {
   /* UNISOC Feature Porting: Vibrate when call connected or disconnected feature. @{ */
    ...
    private static final List<String> CHINA_MOBILE_NUMBER = new ArrayList<String>() {
        {
            this.add("0532100860086");
        }
    };
   ...
   /*UNISOC CMCC featrure porting: FL0108130015 not show hold button when in the videocall @{*/
    ...
    public static void vibrateForCallStateChange(Context context, DialerCall call, String preferenceName) {
        ...
        try {
            ...
            vibrate = sp.getBoolean(preferenceName, true);
        } catch (Exception e) {
   ...
   /* UNISOC Feature Porting: Flip to silence from incoming calls. @{ */
    ...
    //UNISOC: add for bug1510544 video customer service
    public static boolean isSupportVideoCustomerServiceAndNumbers(Context context, DialerCall call) {
        boolean configValue = false;
        if (CHINA_MOBILE_NUMBER != null && call != null){
            configValue = isSupportVideoCustomerService(context, call)
                    && CHINA_MOBILE_NUMBER.contains(call.getNumber());
        }
        LogUtil.i("InCallUtils.isSupportVideoCustomerServiceAndNumbers","Value =" + configValue);
        return configValue;
    }

    public static boolean isSupportVideoCustomerService(Context context, DialerCall call) {
        if (call != null && context != null) {
            CarrierConfigManager configManager = (CarrierConfigManager) context
                    .getSystemService(Context.CARRIER_CONFIG_SERVICE);
            PhoneAccountHandle accountHandle = call.getAccountHandle();
            int currentSubid = getSubIdForPhoneAccountHandle(context, accountHandle);
            LogUtil.d("InCallUtils.isSupportVideoCustomerService","currentSubid =" + currentSubid
                    + ",accountHandle.ID =" + (accountHandle == null ? "null" : accountHandle.getId()));
            if (-1 == currentSubid) {
                LogUtil.d("InCallUtils.isSupportVideoCustomerService","getcurrentSubid failed");
                return  false;
            }
            if (configManager != null && configManager.getConfigForSubId(currentSubid) != null) {
                boolean configValue = (configManager.getConfigForSubId(currentSubid)
                        .getBoolean(CarrierConfigManagerEx.KEY_SUPPORT_VIDEO_CUSTOMER_SERVICE));
                LogUtil.i("InCallUtils.isSupportVideoCustomerService","Value =" + configValue);
                return configValue;
            } else {
                LogUtil.e("InCallUtils","isSupportVideoCustomerService getConfigForSubId = null");
            }
        }
        return false;
    }
