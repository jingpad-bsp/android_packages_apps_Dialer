...
public class CallerAddressPlugin extends CallerAddressHelper {
    ...
    private static final String CTCC_OPERATOR_NUMBER = "10000";//UNISOC:add for bug1381411
    ...
    public CallerAddressPlugin(Context context) {
        mContext = context;
    }
    ...
    public String getCallerAddress(Context context, String number) {
        ...
        if (!TextUtils.isEmpty(number)
                ...
                && call != null //SPRD: Add for bug1003542
                && !call.isEmergencyCall()  //!PhoneNumberUtils.isEmergencyNumber(number)
                && !isOperatorNumber(number)) {
            try {
                newDescription = mContext.getString(R.string.unknown_area);
    ...
    public static boolean isOperatorNumber(String number) {
        if ((number.length() >= 5 && (number.startsWith(CMCC_OPERATOR_NUMBER)
                ...
                || number.startsWith(CTCC_OPERATOR_NUMBER)))) {
            return true;
        }
        return false;
    }
